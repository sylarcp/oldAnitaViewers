...
#pragma implementation "GpsPosViewer.h"
#endif
...
IMPLEMENT_CLASS(GpsPosViewer,wxDialog)
BEGIN_EVENT_TABLE(GpsPosViewer,wxDialog)
 EVT_BUTTON(TERM_CLEAR_BUTTON_ID, GpsPosViewer::OnClear)
 EVT_BUTTON(SAVE_FILE_ID, GpsPosViewer::SaveFile)
 //EVT_CHOICE(wxID_ANY,GpsPosViewer::FileSelect)
END_EVENT_TABLE()

// Constructor
GpsPosViewer::GpsPosViewer(wxWindow *parent){
 Create(parent,wxID_ANY,_T("Gps Postion Viewer"),wxDefaultPosition,wxSize(400,300),
 ...
 // wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
  wxStaticText* staticText1 = new wxStaticText(this, wxID_STATIC,wxT("code  checksum  feByte  numBytes  verId"),
 					     wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
  wxStaticText* staticText2 = new wxStaticText(this, wxID_STATIC,wxT("unixTime  unixTimeUs  latitude  longitude  altitude"),
 					     wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
  wxStaticText* staticText3 = new wxStaticText(this, wxID_STATIC,wxT("numSats  speedInKnots  verticalVelocity  trueCourse"),
 					     wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
 ...
 wxButton *clearButton=new wxButton(this,TERM_CLEAR_BUTTON_ID,_("Clear"));
 wxButton *saveFileButton=new wxButton(this,SAVE_FILE_ID,_("Save File"));
/*
...
GpsPosViewer::~GpsPosViewer(){}

void GpsPosViewer::GpsPosData(const GpsG12PosStruct_t &pos){
  //(*m1_display).Clear();
  //(*m1_display)<<"This is a data viewer."<<"\n";
//////////////////////////////// june 27 2008 9:15 am/////////////////////////////////////////
 (*m1_display)<<int(pos.gHdr.code)<<_T(' ')<<double(pos.gHdr.checksum)<<_T(' ')<<int(pos.gHdr.feByte)<<_T(' ')<<int(pos.gHdr.numBytes)<<_T(' ')<<int(pos.gHdr.verId)<<_T("\n");
 (*m1_display)<<double(pos.unixTime)<<_T(' ')<<double(pos.unixTimeUs)<<_T(' ')<<pos.latitude<<_T(' ')<<pos.longitude<<_T(' ')<<pos.altitude<<_T("\n");
 (*m1_display)<<double(pos.numSats)<<_T(' ')<<pos.speedInKnots<<_T(' ')<<pos.verticalVelocity<<_T(' ')<<pos.trueCourse<<_T("\n");
...
void GpsPosViewer::OnClear(wxCommandEvent &WXUNUSED(event)){
  m1_display->Clear();
}
void GpsPosViewer::SaveFile( wxCommandEvent &WXUNUSED(event) ){
  wxFileDialog *dlg = new wxFileDialog(this,_T("Save options file"),_T(""),_T("G12PosData.txt"),_T("Aview options(*.txt)|*.txt"),wxSAVE);
